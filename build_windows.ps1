# Snappy Web Agent - Windows PowerShell Build Script
# Creates x64 and x86 binaries and packages them into an MSI installer with Windows Service setup
...
function Show-Help {
    Write-Host @"
Snappy Web Agent - Windows Build Script

Usage: .\build_windows.ps1 [OPTIONS]

Options:
  -Help        Show this help message
  -Clean       Only clean build artifacts (don't build)
  -SkipBuild   Skip the Rust build process (useful for installer-only builds)

Examples:
  .\build_windows.ps1          # Full build process
  .\build_windows.ps1 -Clean   # Clean only
  .\build_windows.ps1 -Help    # Show help

Requirements:
  - Rust toolchain with MSVC targets
  - WiX Toolset v3.x
...
Write-Info "Starting Windows universal build process..."
...
function Test-Dependencies {
    ...
    if (!(Get-Command cargo -ErrorAction SilentlyContinue)) {
        $errors += "Cargo is not installed. Please install Rust from https://rustup.rs/"
    }
    
    if (!(Get-Command rustup -ErrorAction SilentlyContinue)) {
        $errors += "Rustup is not installed. Please install Rust from https://rustup.rs/"
    }
    
    if (!(Get-Command candle.exe -ErrorAction SilentlyContinue)) {
        $errors += "WiX Toolset is not installed. Please install from https://wixtoolset.org/releases/"
    }
    
    if (!(Get-Command light.exe -ErrorAction SilentlyContinue)) {
        $errors += "WiX Light tool is not found. Please ensure WiX is in PATH."
    }
...
function Invoke-Clean {
    ...
    $itemsToRemove = @(
        ...
        "target\x86_64-pc-windows-gnu\release\$APP_NAME.exe",
        "target\i686-pc-windows-msvc\release\$APP_NAME.exe",
        $BUILD_DIR,
        $DIST_DIR,
        $WIX_DIR,
        "*.msi",
...
function Add-RustTargets {
    Write-Info "Adding Rust targets for cross-compilation..."
    & rustup target add x86_64-pc-windows-msvc
    & rustup target add x86_64-pc-windows-gnu
    Write-Success "Rust targets added"
}

# Build functions
function Build-Target {
    param($Target, $Description)
    Write-Info "Building for $Description..."
    & cargo build --release --target $Target
    if ($LASTEXITCODE -ne 0) { Write-Error "$Description build failed"; exit 1 }
    Write-Success "$Description build completed"
}
...
function New-BuildStructure {
    ...
    $x64Binary = "target\x86_64-pc-windows-msvc\release\$APP_NAME.exe"
    ...
    if (Test-Path $x64Binary) {
        ...
        if (Test-Path "target\x86_64-pc-windows-msvc\release\") {
            ...
            Get-ChildItem "target\x86_64-pc-windows-msvc\release\" | Write-Host
        }
...
try {
    ...
    if (!$SkipBuild) {
        Add-RustTargets
        Build-Target "x86_64-pc-windows-msvc" "x64 (Windows 64-bit)"
    # 32-bit build removed
    }
    
    # Always create build structure (copies existing binaries)
    New-BuildStructure
